...
public class JwtUtils {
    ...
    public List<UserLoginResponseDto> getObjectFromToken(String token, String name) {
        return (List<UserLoginResponseDto>) Jwts.parserBuilder()
                .setSigningKey(Keys.hmacShaKeyFor(secretKey.getBytes()))
                .build()
                .parseClaimsJws(token).getBody().get(name);
    }

    public String generateAccessTokenObject(UserLoginResponseDto object) {
        Map<String, Object> claims = new HashMap<>();
